...
export default function Card({ image, name, role, group }) {
  const [img, setImg] = useState(image || DEFAULT_IMAGE);

  return (
    <div
      onClick={() => alert(`Hello ${name} from ${group}`)}
      className="group w-75 overflow-hidden rounded-xl bg-white pb-3 text-center transition-all duration-300 ease-in-out hover:-translate-y-2 hover:cursor-pointer hover:shadow-lg/25"
    >
      <img
